...
class AutoUpdater:
    ...
    @staticmethod
    def handle_esky_status(message,progress_bar):
        progress = None
    ...
    def run():
        if getattr(sys, "frozen", False):
            ...
            if updater.find_update():
                reply = messagebox.askquestion("Out of date","Update available; would you like to update Guerillo?")
                if reply:
                    root = tk.Tk()
                    ...
                    root.mainloop()
                    updater.auto_update(AutoUpdater.handle_esky_status,progress_bar)
                    root.destroy()
                else:
